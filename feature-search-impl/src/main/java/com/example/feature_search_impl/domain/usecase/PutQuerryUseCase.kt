...
class PutQuerryUseCase @Inject constructor(
    private val mapper: MapFromDataToDomain,
    private val repository: SearchRepository,
    private val corutineDispatcher: CoroutineDispatcher
) {
    suspend operator fun invoke(text:String):QuerryDomainModel {
        return withContext(corutineDispatcher) {
            mapper.mapQuerryFromDataToDomain(repository.saveQuerry(text))
        }
    }
}